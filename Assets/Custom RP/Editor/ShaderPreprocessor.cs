...
using UnityEngine.Rendering.Custom;
using UnityEngine.Rendering;

namespace UnityEditor.Rendering.Custom
{
    [Flags]
    enum ShaderFeatures
    {
        MainLight = (1 << 0),
        MainLightShadows = (1 << 1), //MainLightShadowCasterPass.Setup方法返回true 则开启
        AdditionalLights = (1 << 2),
        AdditionalLightShadows = (1 << 3), //pipelineAsset.supportsAdditionalLightShadows
        VertexLighting = (1 << 4),
        SoftShadows = (1 << 5), //pipelineAsset.supportsSoftShadows && 开启主光源阴影或者点光源阴影
        MixedLighting = (1 << 6),//pipelineAsset.supportsMixedLighting
        TerrainHoles = (1 << 7),
    ...
    internal class ShaderPreprocessor : IPreprocessShaders
    {
        ...
        int m_TotalVariantsInputCount;//原始变体数量
        int m_TotalVariantsOutputCount;//过滤后的实际变体数量
        ...
        //metapass不需要，如果没有开启MainLightShadows和AdditionalLightShadows，那么ShadowCaster不需要
        ...
        {
            // MainLightShadows没开启，下MainLightShadows，m_CascadeShadows不应该开启
            ...
            //ShaderFeatures.MixedLighting不开启，则m_LightmapShadowMixing和m_ShadowsShadowMask无效
...
#endif
            CustomRenderPineAsset urpAsset = GraphicsSettings.renderPipelineAsset as CustomRenderPineAsset;
...
#endif
    {
        ...
        {
            List<CustomRenderPineAsset> urps = new List<CustomRenderPineAsset>();
            urps.Add(GraphicsSettings.defaultRenderPipeline as CustomRenderPineAsset);
            for(int i = 0; i < QualitySettings.names.Length; i++)
            {
                urps.Add(QualitySettings.GetRenderPipelineAssetAt(i) as CustomRenderPineAsset);
            }

            // Must reset flags.
            _supportedFeatures = 0;
            foreach (CustomRenderPineAsset urp in urps)
            {
                if (urp != null)
                {
        ...
        private static ShaderFeatures GetSupportedShaderFeatures(CustomRenderPineAsset pipelineAsset)
        {
